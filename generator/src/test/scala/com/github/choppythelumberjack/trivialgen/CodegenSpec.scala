package com.github.choppythelumberjack.trivialgen.codegen
...
import com.github.choppythelumberjack.trivialgen.model.StereotypingService._

class CodegenSpec extends FreeSpec with Matchers {
  ...
  def assertStandardObject(objectCode:String, objectName:String, ccName:String, querySchemas: Seq[QuerySchema]) = {
    ...
    ob match {
      case q"$mods object $otname extends { ..$earlydefns } with ..$parents { $self => ..$body }" => {
        assert(otname.toString == objectName, s"Object name $otname in corret")
        ...
        querySchemas.zip(body).foreach({case (schema, methodTree) => {
          methodTree match {
